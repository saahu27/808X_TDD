...
double controller::PID::RunController(double targetVel, double currentVel) {
  while (std::fabs(targetVel - currentVel) > GetterTolerance()) {
    currentVel += ComputeVel(targetVel, currentVel);
    std::cout << "Current state: " << currentVel << "\n";
  }
  return currentVel;
}

double controller::PID::GetterTolerance() { return tolerance_; }

double controller::PID::returndt() { return dt_; }
